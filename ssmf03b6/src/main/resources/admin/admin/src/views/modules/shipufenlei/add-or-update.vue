<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="食谱分类" prop="shipufenlei">
          <el-input v-model="ruleForm.shipufenlei" 
              placeholder="食谱分类" clearable  :readonly="ro.shipufenlei"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	shipufenlei : false,
            ...
            ruleForm: {
                          ...
                          rules: {
                  shipufenlei: [
                            { required: true, message: '食谱分类不能为空', trigger: 'blur' },
                                    	                                                              ],
              }
    };
  },
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `shipufenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shipufenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
